...
int main() {
	...
	while (1) {
		...
		while (1) {
			if (flag == 0) {
				bytesSent = send(s_new, (char*)&msg[0], msg.size(), 0); //посылаем сообщение
				if (bytesSent == SOCKET_ERROR) {
					//cout << "Error sending data to client: " << WSAGetLastError() << endl;
					break;
				} //проверяет ошибки при отправке данных клиенту
				bytesReceived = recv(s_new, (char*)buf, BUF_SIZE, 0); //получаем новое сообщение
				...
				bytesReceived = recv(s_new, (char*)&B, sizeof(B), 0);
				if (bytesReceived == SOCKET_ERROR) {
					//cout << "Error receiving data from client: " << WSAGetLastError() << endl;
					break;
				}
				...
				bytesSent = send(s_new, (char*)&answer, sizeof(answer), 0);
				if (bytesSent == SOCKET_ERROR) {
					//cout << "Error sending data to client: " << WSAGetLastError() << endl;
					break;
				} //проверяет ошибки при отправке данных клиенту
